...
如果还没有明确场景，建议从最简单的抽取式 memory 开始。
先把“事实、偏好、决策”三类写清楚，再谈图结构和自主换页。

## 9 最后

Agent memory 不应该一开始就做成“无限记忆”。

更稳的起点是先分清三件事：Context 负责这次模型能看到什么，State 负责任务跑到哪里，Memory 负责哪些信息值得跨请求保留。

大多数系统的问题不是模型记不住，而是没有决定哪些信息该保留、何时过期、检索出来后该不该注入。先把事实、偏好、决策三类做清楚，再逐步考虑图结构、反思记忆和更复杂的方案。
